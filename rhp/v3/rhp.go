...
func (sh *SessionHandler) handleHostStream(s *rhp3.Stream, log *zap.Logger) {
	...
	log = log.Named(rpc.String()).With(zap.String("rpcID", hex.EncodeToString(frand.Bytes(4))))
...
func (sh *SessionHandler) Serve() error {
	for {
		...
		go func() {
			...
			log := sh.log.With(zap.String("sessionID", hex.EncodeToString(frand.Bytes(4))), zap.String("peerAddress", conn.RemoteAddr().String()))
...
func NewSessionHandler(l net.Listener, hostKey types.PrivateKey, chain ChainManager, syncer Syncer, wallet Wallet, accounts AccountManager, contracts ContractManager, registry RegistryManager, storage StorageManager, settings SettingsReporter, log *zap.Logger) (*SessionHandler, error) {
	sh := &SessionHandler{
		privateKey: hostKey,
