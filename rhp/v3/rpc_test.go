...
import (
	...
	proto2 "go.sia.tech/hostd/v2/internal/testutil/rhp/v2"
	proto3 "go.sia.tech/hostd/v2/internal/testutil/rhp/v3"
...
func setupRHP3Host(t *testing.T, node *testutil.HostNode, hostKey types.PrivateKey, maxStorage uint64, log *zap.Logger) (*rhp2.SessionHandler, *rhp3.SessionHandler) {
	...
	sh2 := rhp2.NewSessionHandler(rhp2Listener, hostKey, node.Chain, node.Wallet, node.Contracts, node.Settings, node.Volumes, log.Named("rhp2"))
	t.Cleanup(func() { sh2.Close() })
	go sh2.Serve()

	sh3 := rhp3.NewSessionHandler(rhp3Listener, hostKey, node.Chain, node.Wallet, node.Accounts, node.Contracts, node.Registry, node.Volumes, node.Settings, log.Named("rhp3"))
