...
func setupRHP3Host(t *testing.T, node *testutil.HostNode, hostKey types.PrivateKey, maxStorage uint64, log *zap.Logger) (*rhp2.SessionHandler, *rhp3.SessionHandler) {
	...
	sh2, err := rhp2.NewSessionHandler(rhp2Listener, hostKey, rhp3Listener.Addr().String(), node.Chain, node.Syncer, node.Wallet, node.Contracts, node.Settings, node.Volumes, log.Named("rhp2"))
	if err != nil {
		t.Fatal(err)
	}
	...
	sh3, err := rhp3.NewSessionHandler(rhp3Listener, hostKey, node.Chain, node.Syncer, node.Wallet, node.Accounts, node.Contracts, node.Registry, node.Volumes, node.Settings, log.Named("rhp3"))
	if err != nil {
		t.Fatal(err)
	}
