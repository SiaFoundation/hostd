...
import (
	...
	rpc2 "go.sia.tech/hostd/v2/internal/testutil/rhp/v2"
...
func TestSettings(t *testing.T) {
	...
	sh := rhp2.NewSessionHandler(l, hostKey, node.Chain, node.Wallet, node.Contracts, node.Settings, node.Volumes, log)
	defer sh.Close()
	go sh.Serve()
