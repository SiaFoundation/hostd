...
func (tx *sectorUpdateTransaction) AddSectorMetadata(root storage.SectorRoot, loc storage.SectorLocation) error {
	...
	_, err := tx.tx.ExecContext(context.Background(), query, valueHash(root), valueHash(loc.Volume), loc.Index)
	return err
}
...
func (ss *SectorStore) SectorLocation(root storage.SectorRoot) (loc storage.SectorLocation, release func() error, err error) {
	err = ss.db.exclusiveTransaction(context.Background(), func(ctx context.Context, tx txn) error {
		err = ss.db.db.QueryRow(`SELECT volume_id, volume_index FROM volume_sectors WHERE sector_root=?;`, root).Scan(&loc.Volume, &loc.Index)
		if err != nil {
...
func (ss *SectorStore) Prune() error {
	_, err := ss.db.db.Exec(`UPDATE volume_sectors vs SET vs.sector_root=null
...
func (ss *SectorStore) Update(updateFn func(storage.SectorUpdateTransaction) error) error {
	return ss.db.exclusiveTransaction(context.Background(), func(ctx context.Context, tx txn) error {
		return updateFn(&sectorUpdateTransaction{tx})
	})
}

func lockSector(tx txn, loc storage.SectorLocation) error {
	_, err := tx.ExecContext(context.Background(), `UPDATE volume_sectors SET locks=locks+1 WHERE volume_id=? AND volume_index=?`, valueHash(loc.Volume), loc.Index)
	return err
}

func unlockSectorFn(db *Store, loc storage.SectorLocation) func() error {
	return func() error {
		return db.exclusiveTransaction(context.Background(), func(ctx context.Context, tx txn) error {
			_, err := tx.ExecContext(ctx, `UPDATE volume_sectors SET locks=locks-1 WHERE volume_id=? AND volume_index=? AND locks > 0`, valueHash(loc.Volume), loc.Index)
			return err
		})
	}
}
