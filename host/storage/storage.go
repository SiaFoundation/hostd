...
func (vm *VolumeManager) ReadSector(root types.Hash256, offset, length uint64) ([]byte, []types.Hash256, error) {
	...
	case offset+length > proto4.SectorSize:
		return nil, nil, fmt.Errorf("read exceeds sector bounds")
	...
	if len(meta.CachedSubtrees) == 0 {
		...
		if err != nil {
			...
			return nil, nil, fmt.Errorf("read length mismatch: expected %d, got %d", length, len(sector))
		}
		cachedSubtrees := proto4.CachedSectorSubtrees((*[proto4.SectorSize]byte)(sector))
		segmentStart, segmentEnd := proto4.SectorSubtreeRange(offset/proto4.LeafSize, (offset+length)/proto4.LeafSize)
		proof := proto4.BuildSectorProof(sector[segmentStart*proto4.LeafSize:segmentEnd*proto4.LeafSize], offset/proto4.LeafSize, (offset+length)/proto4.LeafSize, cachedSubtrees)
	...
	buf, err := vm.readLocation(meta.Location, segmentStart*proto4.LeafSize, segmentLength)
	if err != nil {
	...
	vm.log.Debug("read sector segment from disk", zap.String("root", root.String()), zap.Uint64("segmentStart", segmentStart), zap.Uint64("segmentEnd", segmentEnd), zap.Uint64("requestedStart", start), zap.Uint64("requestedEnd", end))
	return buf[(start-segmentStart)*proto4.LeafSize:][:length], proto4.BuildSectorProof(buf, start, end, meta.CachedSubtrees), nil
}
