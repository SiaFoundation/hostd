...
ON CONFLICT (id) DO UPDATE SET (settings_revision, 
	...
	EXCLUDED.max_account_age, EXCLUDED.price_table_validity, EXCLUDED.max_contract_duration,
	EXCLUDED.window_size, EXCLUDED.ingress_limit, EXCLUDED.egress_limit, EXCLUDED.registry_limit);`
	...
	_, err := s.exec(query, settings.AcceptingContracts,
		...
		settings.AccountExpiry, settings.PriceTableValidity, settings.MaxContractDuration,
		settings.WindowSize, settings.IngressLimit, settings.EgressLimit, settings.MaxRegistryEntries)
	return err
}
