...
func (s *Store) Settings() (config settings.Settings, err error) {
	...
	ingress_limit, egress_limit, registry_limit, ddns_provider, ddns_update_v4, ddns_update_v6, ddns_opts, sectors_to_cache
FROM host_settings;`
	err = s.queryRow(query).Scan(&config.Revision, &config.AcceptingContracts,
		...
		&config.DDNS.Provider, &config.DDNS.IPv4, &config.DDNS.IPv6, &dyndnsBuf, &config.SectorsToCache)
...
func (s *Store) UpdateSettings(settings settings.Settings) error {
	const query = `INSERT INTO host_settings (id, settings_revision, 
		...
		egress_limit, registry_limit, ddns_provider, ddns_update_v4, ddns_update_v6, ddns_opts, sectors_to_cache) 
...
ON CONFLICT (id) DO UPDATE SET (settings_revision, 
	...
	EXCLUDED.ddns_update_v4, EXCLUDED.ddns_update_v6, EXCLUDED.ddns_opts, EXCLUDED.sectors_to_cache);`
	var dnsOptsBuf []byte
	...
	return s.transaction(func(tx txn) error {
		_, err := tx.Exec(query, settings.AcceptingContracts,
			...
			settings.DDNS.Provider, settings.DDNS.IPv4, settings.DDNS.IPv6, dnsOptsBuf, settings.SectorsToCache) //Update the parameter for the new column
		if err != nil {
