...
func (s *Store) Settings() (config settings.Settings, err error) {
	...
	ingress_limit, egress_limit, ddns_provider, ddns_update_v4, ddns_update_v6, ddns_opts, sector_cache_size
FROM host_settings;`
	err = s.queryRow(query).Scan(&config.Revision, &config.AcceptingContracts,
		...
		&config.IngressLimit, &config.EgressLimit, &config.DDNS.Provider,
...
func (s *Store) UpdateSettings(settings settings.Settings) error {
	const query = `INSERT INTO host_settings (id, settings_revision, 
		...
		egress_limit, ddns_provider, ddns_update_v4, ddns_update_v6, ddns_opts, sector_cache_size) 
...
ON CONFLICT (id) DO UPDATE SET (settings_revision, 
	...
	EXCLUDED.ingress_limit, EXCLUDED.egress_limit, EXCLUDED.ddns_provider, 
	...
	return s.transaction(func(tx txn) error {
		_, err := tx.Exec(query, settings.AcceptingContracts,
			...
			settings.IngressLimit, settings.EgressLimit, settings.DDNS.Provider,
			settings.DDNS.IPv4, settings.DDNS.IPv6, dnsOptsBuf, settings.SectorCacheSize)
		if err != nil {
