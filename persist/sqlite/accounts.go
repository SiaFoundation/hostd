...
func (s *Store) RHP4CreditAccounts(deposits []proto4.AccountDeposit, contractID types.FileContractID, revision types.V2FileContract, usage contracts.V2Usage) (balances []types.Currency, err error) {
	err = s.transaction(func(tx *txn) error {
		...
		updateBalanceStmt, err := tx.Prepare(`INSERT (account_id, balance, expiration_timestamp) VALUES ($1, $2, $3) ON CONFLICT (account_id) DO UPDATE SET balance=EXCLUDED.balance, expiration_timestamp=EXCLUDED.expiration_timestamp`)
		if err != nil {
			return fmt.Errorf("failed to prepare update balance statement: %w", err)
		}
		defer updateBalanceStmt.Close()

		getFundingAmountStmt, err := tx.Prepare(`SELECT amount FROM contract_account_funding WHERE contract_id=$1 AND account_id=$2`)
		if err != nil {
			return fmt.Errorf("failed to prepare get funding amount statement: %w", err)
		}
		defer getFundingAmountStmt.Close()

		updateFundingAmountStmt, err := tx.Prepare(`INSERT INTO contract_account_funding (contract_id, account_id, amount) VALUES ($1, $2, $3) ON CONFLICT (contract_id, account_id) DO UPDATE SET amount=EXCLUDED.amount`)
		if err != nil {
		...
		for _, deposit := range deposits {
			...
			if _, err := updateBalanceStmt.Exec(encode(deposit.Account), encode(balance), encode(time.Now().Add(90*24*time.Hour))); err != nil {
				return fmt.Errorf("failed to update balance: %w", err)
			}
			balances = append(balances, balance)

			var fundAmount types.Currency
			if err := getFundingAmountStmt.QueryRow(contractID, deposit.Account).Scan(decode(&fundAmount)); err != nil && !errors.Is(err, sql.ErrNoRows) {
				return fmt.Errorf("failed to get funding amount: %w", err)
			}
			fundAmount = fundAmount.Add(deposit.Amount)
			if _, err := updateFundingAmountStmt.Exec(contractID, deposit.Account, encode(fundAmount)); err != nil {
