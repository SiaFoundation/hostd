...
WHERE c.contract_id=?`
		...
		err := tx.QueryRow(query, encode(contractID)).Scan(decode(&ele.V2FileContract), decode(&ele.LeafIndex), decode(&ele.MerkleProof), decode(&basis))
		if errors.Is(err, sql.ErrNoRows) {
			return contracts.ErrNotFound
		}
...
func incrementV2ContractUsage(tx *txn, dbID int64, usage contracts.V2Usage) error {
	...
	var existing contracts.V2Usage
...
func updateV2ContractUsage(tx *txn, contractDBID int64, usage contracts.V2Usage) error {
...
func reviseV2Contract(tx *txn, id types.FileContractID, revision types.V2FileContract, usage contracts.V2Usage) (int64, error) {
...
func (s *Store) ReviseV2Contract(id types.FileContractID, revision types.V2FileContract, roots []types.Hash256, usage contracts.V2Usage) error {
	return s.transaction(func(tx *txn) error {
