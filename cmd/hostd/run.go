...
func runRootCmd(ctx context.Context, cfg config.Config, walletKey types.PrivateKey, instantSync bool, log *zap.Logger) error {
	...
	if instantSync && !consensusExists {
		...
		} else if hasContracts && contractHeight < checkpoint.Height {
			blocksPerDay := uint64(24 * time.Hour / network.BlockInterval)
			checkpoint, err = exp.TipHeight(ctx, min(contractHeight-blocksPerDay, 0))
			if err != nil {
				return fmt.Errorf("failed to get tip height from explorer: %w", err)
			}
		}

		if checkpoint.Height < network.HardforkV2.RequireHeight {
