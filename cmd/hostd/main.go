...
func main() {
	...
	web := http.Server{
		Handler: webRouter{
			api: auth(api.NewServer(cfg.Name, hostKey.PublicKey(), node.a, node.g, node.cm, node.tp, node.contracts, node.accounts, node.storage, node.sessions, node.metrics, node.settings, node.w, node.hooks, log.Named("api"))),
			ui:  hostd.Handler(),
		},
